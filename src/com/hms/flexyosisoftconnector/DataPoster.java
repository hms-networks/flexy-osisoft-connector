  ...
  public void run() {
    ...
    while (shouldRun) {
      ...
      for (int tagIndex = 0; tagIndex < OSIsoftConfig.tags.size(); tagIndex++) {
        ...
        for (int dataPointIndex = 0; dataPointIndex < ((ArrayList) dataPoints.get(tagIndex)).size();
            dataPointIndex++) {
          if (((ArrayList) dataPoints.get(tagIndex)).get(dataPointIndex) != null) {
            switch (communicationType) {
              case OSIsoftConfig.omf:
                String tagValue = ((DataPoint) ((ArrayList) dataPoints.get(tagIndex))
                    .get(dataPointIndex)).getValueString();
                String timeStamp = ((DataPoint) ((ArrayList) dataPoints.get(tagIndex))
                    .get(dataPointIndex)).timestamp;
                ...
                break;
              case OSIsoftConfig.piwebapi:
                OSIsoftServer.addPointToBatch(((Tag) OSIsoftConfig.tags.get(tagIndex)),
                    (DataPoint) ((ArrayList) dataPoints.get(tagIndex)).get(dataPointIndex));
            ...
            Logger.LOG_ERR("Null data point encountered");
          }
        }
        switch (communicationType) {
          case OSIsoftConfig.omf:
      ...
      if (pointsAdded > 0) {
        ...
        if (retval) {
          ...
          for (int tagIndex = 0; tagIndex < OSIsoftConfig.tags.size(); tagIndex++) {
            ((Tag) OSIsoftConfig.tags.get(tagIndex))
                .removeDataPoints((ArrayList) dataPoints.get(tagIndex));
          }
        }
      }
